...
uint64_t OP_OP     = 51;  // 0110011, R format (ADD, SUB, MUL, DIVU, REMU, SLTU, SLL, SRL, OR, AND)
...
void init_disassembler() {
  MNEMONICS = smalloc((AND + 1) * sizeof(uint64_t*));
...
uint64_t is_bitwise_and() {
  return symbol == SYM_BITWISEAND;
}
...
uint64_t compile_bitwise_or() {
  ...
  // type of bitwise or is grammar attribute
  return ltype;
}
...
uint64_t compile_bitwise_and() {
  ...
  // type of bitwise and is grammar attribute
  return ltype;
}
...
uint64_t compile_shift() {
  ...
  // type of shift is grammar attribute
  return ltype;
}
...
void selfie_compile() {
  ...
  while (link) {
    if (number_of_remaining_arguments() == 0)
      ...
      printf("%s: %lu assignments, %lu while, %lu if, %lu calls, %lu for, %lu return, \n", selfie_name,
...
uint64_t get_total_number_of_instructions() {
  return ic_lui + ic_addi + ic_xori + ic_add + ic_sub + ic_mul + ic_divu + ic_remu + ic_sltu + ic_sll + ic_srl + ic_or + ic_and + ic_load + ic_store + ic_beq + ic_jal + ic_jalr + ic_ecall;
}

uint64_t get_total_number_of_nops() {
  return nopc_lui + nopc_addi + nopc_xori + nopc_add + nopc_sub + nopc_mul + nopc_divu + nopc_remu + nopc_sltu + nopc_sll + nopc_srl + nopc_or + nopc_and + nopc_load + nopc_store + nopc_beq + nopc_jal + nopc_jalr;
}
...
void do_sll() {
  ...
  if (rd != REG_ZR) {
    // semantics of sll
    next_rd_value = left_shift(*(registers + rs1), *(registers + rs2) % WORDSIZEINBITS);
...
void do_srl() {
  ...
  if (rd != REG_ZR) {
    // semantics of srl
    next_rd_value = right_shift(*(registers + rs1), *(registers + rs2) % WORDSIZEINBITS);
...
void decode() {
  ...
  if (opcode == OP_IMM) {
    ...
    if (IS64BITTARGET) {
      ...
      is = STORE;
  } else if (opcode == OP_OP) { // could be ADD, SUB, MUL, DIVU, REMU, SLTU, SLL, SRL, OR, AND
    decode_r_format();
...
uint64_t selfie(uint64_t extras) {
  if (number_of_remaining_arguments() == 0)
    ...
    printf("%s: This is Justin Klein's Selfie!\n", selfie_name);
